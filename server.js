...
function generateResponse(input, type, data) {
  ...
  const prefix =
    friendly.prefixes[Math.floor(Math.random() * friendly.prefixes.length)];
  const suffix =
    friendly.suffixes[Math.floor(Math.random() * friendly.suffixes.length)];
  ...
  if (type === 'math') {
    ...
    const baseResponses =
      trainingData.conversations.find((c) =>
...
app.post('/chat', async (req, res) => {
  ...
  try {
    ...
    const pythonProcess = spawn(
      pythonExecutable,
      [path.join(__dirname, 'src', 'chat_model.py'), message],
      {
        env: {
          ...process.env,
    ...
    res
      .status(500)
      .send("😅 Oops! Had a little trouble there. Could you try again?");
  }
});
